...
class App extends Component {
  paginate = (pag) => {
    if (
      pag === "R" &&
      (this.state.pageDisplayed + 1) * this.state.numTodisplay <
        this.state.jb.length
    ) {
      let firstTodisplay =
        (this.state.pageDisplayed + 1) * this.state.numTodisplay;
  ...
  getProducts = () => {
    axios
      ...
      .then((result) => {
        ...
        for (
          let i = 1;
          i < this.state.jb.length / this.state.numTodisplay + 1;
          i++
        ) {
  ...
  dbCustUpdate = (data, id) => {
    const url =
      "https://f8nibhiadf.execute-api.eu-west-2.amazonaws.com/dev/tasks/" + id;
    axios
      .put(url, data)
      .then((result) => {
        // Task if deleted OK
  ...
  async dbAddCustomer(object) {
    let response = await axios.post(
      "https://rxfg014ygk.execute-api.eu-west-2.amazonaws.com/dev/tasks",
      object
    );
    let { data } = response;
    ...
    jb: [
      {
        ...
        sku: "JB01",
        image:
          "https://www.simoncobb.co.uk/jbshop2/images/product_images/doctor_no.jpg",
        description:
...

      {
        ...
        sku: "JB05",
        image:
          "https://www.simoncobb.co.uk/jbshop2/images/product_images/live_twice.jpg",
        description:
          "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas et metus.",
        price: 9.99,
        priceblu: 14.99,
...

      {
        ...
        stockblu: 23,
        synopsis:
          "James Bond is sent to investigate the murder of three British MI6 agents, all of whom have been killed within 24 hours. He discovers the victims were all separately investigating the operations of Dr. Kananga, the dictator of a small Caribbean island, San Monique. He also establishes that Kananga also acts as Mr. Big, a ruthless and cunning American gangster. Upon visiting San Monique, Bond determines that Kananga is producing two tons of heroin and is protecting the poppy fields by exploiting locals' fear of voodoo and the occult. Through his alter ego, Mr. Big, Kananga plans to distribute the heroin free of charge at his Fillet of Soul restaurants, which will increase the number of addicts. Bond is captured by Kananga, but he escapes, killing Kananga and destroying the poppy crop.",
      },
  ...
  render() {
    const pagNums = this.state.paginateNums.map((item, i) => (
      <span>
        <li className="pagContainer" key={this.UUID.toString()}>
          {i === this.state.pageDisplayed && (
            <button
              className="pagButtonselected h2"
              key={this.UUID.toString()}
              onClick={() => {
                this.paginate(i);
              }}
            >
          ...
          <span>
            ...
            {this.state.login && (
              <>
                ...
                >
                  SIGN OUT {this.state.customer.firstName.slice(0, 10)}
                </Button>
                <Button
                  className="buttona"
                  onClick={() => {
                    this.openModal("accountdetails");
                  }}
                >
                  YOUR DETAILS
                </Button>
